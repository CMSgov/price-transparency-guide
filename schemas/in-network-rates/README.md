...
| **provider_groups** | Provider Groups | Array  | The [providers object](#providers-object) defines information about the provider and their associated TIN related to the negotiated price. | No |
| **location** | Location | String  | A fully qualified domain name on where the provider group data can be downloaded. The file must validate against the requirements found in the [provider reference](https://github.com/CMSgov/price-transparency-guide/tree/external-references/schemas/provider-reference). Examples can be found [here](https://github.com/CMSgov/price-transparency-guide/blob/external-references/examples/in-network-rates/in-network-rates-fee-for-service-single-plan-sample.json#L10-L13) that would link to a
valid provider reference file such as one found [here](https://github.com/CMSgov/price-transparency-guide/blob/external-references/examples/provider-reference/provider-reference.json). | No |

##### Additional Notes Concerning `provider_groups`, `location`
